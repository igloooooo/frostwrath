...
static const uint8 REFORG_PRICE = 50;
...
static void UpdatePlayerReforgeStats(Item* invItem, Player* player, uint32 decrease, uint32 increase) // stat types
{
    ...
    player->ModifyMoney(pProto->SellPrice < (REFORG_PRICE * GOLD) ? (-1*REFORG_PRICE * GOLD) : -(int32)pProto->SellPrice);
...
public:
    ...
    {
        ChatHandler(player->GetSession()).SendSysMessage("This server is running the |cff4CFF00ReforgingNPC |rmodule.");
                ...
                default: // Reforge
                    ...
                    // action = stat type to decrease
                    {
                        if (menu < stat_type_max)
                        {
                            ...
                            {
                                if (player->HasEnoughMoney(invItem->GetTemplate()->SellPrice < (REFORG_PRICE * GOLD) ? (REFORG_PRICE * GOLD) : invItem->GetTemplate()->SellPrice))
                                {
